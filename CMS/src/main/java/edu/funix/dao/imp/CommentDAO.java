...
public class CommentDAO extends AbstractDAO<CommentModel> implements ICommentDAO {
	...
	return query(sql, new CommentMapper(), parentComId);
    }
	...
	sql.append("INSERT INTO tblCOMMENT (ComContent, SubmitBy, SeedingBy, SubmitTo, ReplyTo, Confirm) ");
	sql.append("VALUES (?,?,?,?,?,?)");
	update(sql.toString(), comment.getComContent(), comment.getCreatedBy(), comment.getSeedBy(),
		comment.getSubmitTo(), comment.getReplyTo(), comment.getConfirm() ? 1 : 0);
    }
