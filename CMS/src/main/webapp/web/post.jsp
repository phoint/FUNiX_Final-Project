				...
				<div class="row">
					<div class="col">
						<c:forEach items="${post.comments}" var="comment">
							<c:if
								test="${comment.confirm or comment.createdBy == sessionScope.loginUser.id or sessionScope.userType == 'User'}">
														...
														<form action="<c:url value="/"/>" method="POST">
															<div class="d-flex flex-row align-items-start">
																<img class="rounded-circle"
																	<c:choose>
                                  <c:when test="${not empty sessionScope.loginUser}">src="${sessionScope.loginUser.pictureUrl}"</c:when>
                                  <c:otherwise>src="https://www.gravatar.com/avatar/4f496af955d7eae902d542ee492d4f1?d=mp&f=y"</c:otherwise>
                                  </c:choose> width="40">
										...
										<c:forEach items="${comment.replies}" var="reply">
											<c:if
												test="${reply.confirm or reply.createdBy == sessionScope.loginUser.id or sessionScope.userType == 'User'}">
												<div class="media mt-3">
													<a class="pr-3" href="#"><img class="rounded-circle"
														alt="Bootstrap Media Another Preview"
														src="${reply.author.pictureUrl}" /></a>
													<div class="media-body">
